...
extension ServicesManager: ServiceDelegate {
    ...
    enum OverrideActionError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .negativeDuration:
                return String(format: NSLocalizedString("负持续时间不允许", comment: "Override error description: negative duration error."))
            }
        }
    }
    ...
    enum BolusActionError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidBolus:
                return NSLocalizedString("无效的推注", comment: "Bolus error description: invalid bolus amount.")
            case .missingMaxBolus:
                return NSLocalizedString("在设置中缺少最大允许的推注", comment: "Bolus error description: missing maximum bolus in settings.")
            case .exceedsMaxBolus:
                return NSLocalizedString("超过设置中最大允许的推注", comment: "Bolus error description: bolus exceeds maximum bolus in settings.")
            }
        }
    }
}
