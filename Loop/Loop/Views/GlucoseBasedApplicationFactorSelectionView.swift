...
public struct GlucoseBasedApplicationFactorSelectionView: View {
    ...
    public var body: some View {
        ScrollView {
            VStack(spacing: 10) {
                Text(NSLocalizedString("基于血糖的部分应用", comment: "Title for glucose based partial application experiment description"))
                    .font(.headline)
                    .padding(.bottom, 20)
                ...
                if automaticDosingStrategy == .automaticBolus {
                    ...
                    HStack {
                        Toggle(NSLocalizedString("启用基于血糖的部分应用", comment: "Title for Glucose Based Partial Application toggle"), isOn: $isGlucoseBasedApplicationFactorEnabled)
                        Spacer()
                    }
                    .padding(.top, 20)
                } else {
                    Text(NSLocalizedString("仅当闭环的剂量策略设置为自动推注时，此选项才适用。", comment: "String shown when glucose based partial application cannot be enabled because dosing strategy is not set to Automatic Bolus"))
                }
            }
            .padding()
